...
export default {
  ...
  mounted() {
    this.fetchCountryNameData().then(countryNameData => {
      ...
      this.countries = countryNameData;
    });
    this.fetchCountryValueData().then(countryValueData => {
      this.yearData = countryValueData;
  ...
  methods: {
    ...
    initializeChart() {
      ...
      let yearLabel = chart.plotContainer.children.push(am5.Label.new(root, {
      ...
      let polygonSeries = navMap.series.push(am5map.MapPolygonSeries.new(root, {
      ...
      this.updateSeriesData(this.currentYear);
