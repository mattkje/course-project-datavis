  ...
  methods: {
    ...
    async initializeChart() {
      ...
      yAxis.children.moveValue(am5.Label.new(root, {
        text: "Yearly Co2 Emission Per Capita",
        rotation: -90,
        y: am5.p50,
        centerX: am5.p50,
        fill: am5.color(0xffffff)
      ...
      am5.array.each(this.bubbleSeries.dataItems, (dataItem) => {
        if (dataItem.bullets) {
          ...
          if (bullet) {
            let sprite = bullet.get("sprite");
            if (sprite) {
              if (target.dataItem.dataContext.code === sprite.dataItem.dataContext.continent.continent) {
                sprite.states.applyAnimate("default");
              } else {
                sprite.states.applyAnimate("transparent");
              }
    ...
    handleContinentClick(e) {
      ...
      if (target.dataItem === selectedContinent) {
        ...
        am5.array.each(this.polygonSeries.dataItems, (dataItem) => {
          let mapPolygon = dataItem.get("mapPolygon");
          if (dataItem.uid !== selectedContinent.uid) {
...
document.addEventListener('scroll', function() {
  ...
  sectionHeaders.forEach(header => {
    ...
    header.style.backgroundPosition = `${scrollPercent * 100}% 0`;
  });
}, { passive: true });
</script>
